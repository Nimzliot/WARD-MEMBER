...
| **Ward Fund (Razorpay)** | The Ward Admin starts fundraisers (goal, end date). Residents contribute with **Razorpay in test mode**: in the **APK**, Razorpay's native **Checkout opens inside the app** (order, then signature check, then capture confirmed server-side); on the **web**, a Razorpay payment page opens in a new tab. A payment counts only after the server confirms it with Razorpay (signed callback + status check). Residents can give anonymously and get a receipt. Test UPI `success@razorpay` or card `4111 1111 1111 1111`. |
